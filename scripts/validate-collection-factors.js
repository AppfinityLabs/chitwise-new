...
async function validate() {
    ...
    console.log(`  Mode: ${FIX_MODE ? '🔧 FIX (will update DB)' : '🔍 DRY-RUN (report only)'}`);
    console.log('═══════════════════════════════════════════════════\n');
    ...
    try {
        ...
        const ChitGroup = mongoose.models.ChitGroup || mongoose.model('ChitGroup', ChitGroupSchema);
        ...
        const allSubscriptions = await GroupMember.find({})
            .populate('groupId', 'groupName frequency contributionAmount totalPeriods startDate status allowCustomCollectionPattern')
        ...
        for (const sub of allSubscriptions) {
            ...
            } else if (!['DAILY', 'WEEKLY', 'MONTHLY'].includes(sub.collectionPattern)) {
            ...
            if (!sub.collectionFactor || sub.collectionFactor <= 0) {
                ...
                if (FIX_MODE && group && sub.collectionPattern) {
                    const correctFactor = deriveCollectionFactor(group.frequency, sub.collectionPattern);
            ...
            if (group && sub.collectionPattern && sub.collectionFactor > 0) {
                const expectedFactor = deriveCollectionFactor(group.frequency, sub.collectionPattern);
            ...
            // 4. Verify totalDue
            if (group) {
                const expectedTotalDue = group.contributionAmount * group.totalPeriods * sub.units;
        ...
        for (const [label, items] of categories) {
            if (items.length > 0) {
                ...
                for (const item of items) {
                    console.log(`   - [${item.subId}] ${item.memberName} in "${item.groupName}": ${item.detail}`);
                }

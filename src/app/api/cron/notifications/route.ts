...
import { calculateCurrentPeriod, calculatePaymentStatus, calculateOverdueAmount, calculateDueAmount, isGroupComplete } from '@/lib/utils';
...
export async function GET(request: NextRequest) {
    ...
    const results = {
        ...
        auto: { overdueAlerts: 0, paymentReminders: 0, groupStarted: 0, groupsClosed: 0, skippedDuplicates: 0, pushSent: 0, pushFailed: 0 },
    };
    ...
    try {
        ...
        for (const group of activeGroups) {
            ...
            // ── GROUP AUTO-CLOSURE ────────────────────────
            // Once every period has elapsed, mark the group CLOSED and send a
            // one-time completion notification to its members.
            if (isGroupComplete(group as any)) {
                await ChitGroup.findByIdAndUpdate(group._id, {
                    status: 'CLOSED',
                    currentPeriod: group.totalPeriods,
                });

                const notif = await Notification.create({
                    title: `${group.groupName} has completed`,
                    body: `All ${group.totalPeriods} periods of "${group.groupName}" are complete. The chit group is now closed. Thank you for participating!`,
                ...
                try {
                    ...
                    console.error(`Push GROUP_CLOSED failed for ${group.groupName}:`, e);
                }
                results.auto.groupsClosed++;
                continue; // No reminders for a closed group
            }

            // ── GROUP_STARTED (one-time) ──────────────────
            if (currentPeriod === 1) {
            ...
            for (const sub of subscriptions) {
                ...
                if (paymentStatus === 'OVERDUE' && overdueAmount > 0) {
                    ...
                    const body = interpolateTemplate(
                        'Hi {{memberName}}, you have an overdue payment of ₹{{amount}} for "{{groupName}}" (Period {{period}}). Please pay at the earliest.',
                ...
                if (paymentStatus === 'DUE') {
                    ...
                    const body = interpolateTemplate(
                        'Hi {{memberName}}, your payment of ₹{{amount}} for "{{groupName}}" (Period {{period}}) is due. Please pay on time.',
